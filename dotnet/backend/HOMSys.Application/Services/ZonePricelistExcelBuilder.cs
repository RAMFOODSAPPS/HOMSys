...
/// <summary>
/// Renders a ZonePricelistExportResult into the one-sheet, per-zone-column
/// comparison workbook, styled the same as PricelistExcelBuilder's
/// per-customer version: bold Calibri 10pt header block, 2-row merged column
/// headers, bold category header rows, matching column widths for the fixed
/// columns (A-E).
/// </summary>
public class ZonePricelistExcelBuilder
{
    public byte[] Build(ZonePricelistExportResult result, List<ZonePricelistColumn>? zones = null)
    {
        zones ??= result.Zones;
        ...
        var lastCol = fixedCols + zones.Count * 3;

        // Header block
        ws.Cell(1, 1).Value = "RAM FOOD PRODUCTS, INC.";
        ws.Cell(2, 1).Value = $"Branch: {result.Branch.ToUpperInvariant()}    Effectivity Date: {result.EffectivityDate:MMMM d, yyyy}";
        for (var r = 1; r <= 2; r++)
        {
        ...
        foreach (var zone in zones)
        {
            ...
            ws.Cell(headerRow1, col).Value = $"ZONE {zone.Zone}";
        ...
        foreach (var group in result.Groups)
        {
            ...
            foreach (var line in group.Rows)
            {
                ...
                foreach (var zone in zones)
                {
                    var value = line.ByZone.GetValueOrDefault(zone.Zone);
