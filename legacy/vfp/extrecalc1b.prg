...
set filter to substr(status,1,1) $ "23456789" .and. extwhse
go top
 ...
 *** recalc allocated sta-out
 dclose1a = .f.
 if .not. used("imtr2_hdr")
    use imtr2_hdr in 0 shared
    dclose1a=.t.
 endif
 dclose1b=.f.
 if .not. used("imtr2_det")
    use imtr2_det in 0 shared
    dclose1b = .t.
 endif
 temp1a = newname("d1a")
 temp1b = newname("d1b")
 ...
 copy to &temp1b
 if dclose1a
    m=closedbf("imtr2_hdr")
 endif
 if dclose1b
    m=closedbf("imtr2_det")
 endif
