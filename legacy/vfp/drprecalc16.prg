* drprecalc16.prg from recalc14.prg
* 06202024
...
closehdr = openfile("drp_hdr")
closedet = openfile("drp_det")
...
if month(sysparam.transdate) = month(xdate) .and. year(sysparam.transdate) = year(xdate)
   ...
   if file(meomfl)
      ...
      IMHDR = renfile(XDATE,"PH",VAL(sysparam.bcode))
      IMDET = renfile(XDATE,"PD",VAL(sysparam.bcode)) 
...
repl ec with 2 for trim(doctype) $ "INVOICE,STN,RTS,MIS,MISS,STAO"
...
************************
    ...
    do while doctype=mdoctype .and. docno=mdocno .and. .not. eof() .and. oktocompute
       ...
       if .not. empty(xcustkey)
          ...
          loca for cprodno=mcprodno .and. custkey = xmdrpcust
       endif
      if .not. found()
          append blank
...
m=closetab("drp_hdr",closehdr)
m=closetab("drp_det",closedet)
