...
* same as recalc11.prg 12/04/2003
*
param xeif,xdate

*wait window "this is recalc1 - press any key. xdate ="+dtoc(xdate)
wait window "this is recalc11 press press press"
...
if file("&imhdr") .and. file("&imdet")
   ...
   * CURRENT MONTH
   ...
   USE &TEMPDET ALIAS TEMPDET IN 0 EXCLUSIVE
ENDIF
temppr_recalc15 = newname("dbf")
sele prod4win
copy to &temppr_recalc15
use &temppr_recalc15 alias temppr_recalc15 in 0 excl
sele temppr_recalc15
appe from inactive
...
set rela to cprodno into temppr_recalc15
*************
*m=messagebox("recalc")
*copy to vicdet stru
*use vicdet in 0
...
*       endif
       *********** 
       ...
       msupplier=temppr_recalc15.supplier
       mcprodno=cprodno
       mclass=class        && class in for rsta
       mclassout=classout
       mpieces=temppr_recalc15.pieces
       muc=uc
